...
test('get 3 product details', async ({ page }) => {
  await page.goto('https://www.amazon.in/');  //page navigation
...
test('get 5 product details', async ({ page }) => {
    await page.goto('https://www.amazon.in/');  //page navigation
    await page.waitForTimeout(1000);
  
    await page.locator('input#twotabsearchtextbox').fill('trolley'); //  // Type 'shoes' into the search bar
    ...
    const suggestions = await page.locator('.s-suggestion');
      // Click the 2nd suggestion from the dropdown
    await suggestions.nth(4).click(); // Index is 0-based, so 1 is the second item
    ...
    // Print all collected results together...
    console.log('The list of first 5 Products:', results);
